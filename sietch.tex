...
\normalsize
For the first time, this paper will explain how the ”ITM Attack” (a linkability attack against shielded transactions) works against Zcash Protocol and how Hush is the first cryptocoin with a defensive mitigation against it, called ”Sietch”. Sietch has been running live in production and undergoing iterations of improvement from expert feedback. This is not an academic paper about pipedreams; it describes production code and networks.\\

We begin with a literature review of all known metadata attack methods that can be used against Zcash Protocol blockchains; this includes their estimated attack costs and threat model.

We then describe the ”ITM Attack” which is a specific instance of a new class of metadata attacks against blockchains that the author describes as ”Metaverse Metadata” attacks. Furthermore, we explain Sietch in detail, which was an response to these new attacks. We hope this new knowledge and theory helps cryptocoins increase their defenses against very well-funded adversaries including nation states and chain analysis companies.\\
...
This paper is primarily concerned with \textbf{shielded transaction graphs} which are \textbf{directed acyclic graphs (DAGs)} . A \shielded transaction does not reveal the address of Alice, nor Bob, nor the amount transacted but it does leak a large amount of metadata at the protocol level, which is not rendered by block explorers nor well understood by the industry.
